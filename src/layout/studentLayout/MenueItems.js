...
function MenuItems() {
  ...
  const items = [
    getItem(<NavLink to={`${path}`}>{t('Home')}</NavLink>),
    getItem(<NavLink to={`${path}/course`}>{t('Courses')}</NavLink>),
    getItem(<NavLink to={`${path}/yourCourses`}>{t('Your Course')}</NavLink>),
    getItem(<NavLink to={`${path}/student-fee`}>{t('Fees')}</NavLink>),
